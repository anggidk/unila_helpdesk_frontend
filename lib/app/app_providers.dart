...
final guestCategoriesProvider = FutureProvider<List<ServiceCategory>>((
  ref,
) async {
  return CategoryRepository().fetchGuest();
});
final surveyTemplatesProvider = FutureProvider<List<SurveyTemplate>>((
  ref,
) async {
...
final entityServiceProvider =
    FutureProvider.autoDispose<List<EntityServiceRow>>((ref) async {
      final period = ref.watch(cohortPeriodProvider);
      return ReportRepository().fetchEntityService(period: period, periods: 1);
    });
final reportsChartUsageProvider =
    FutureProvider.autoDispose<List<UsageCohortRow>>((ref) async {
...
final dashboardUsageProvider = FutureProvider.autoDispose<List<UsageCohortRow>>(
  (ref) async {
    return ReportRepository().fetchUsageCohort(period: 'monthly', periods: 6);
  },
);
